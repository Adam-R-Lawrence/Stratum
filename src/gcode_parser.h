...
// commands, numbers, and quoted strings without relying on simple token splitting.
inline GCodeCommand parseLine(std::string_view view) {
    ...
    while (pos < view.length()) {
        // Skip whitespace
        while (pos < view.length() && std::isspace(static_cast<unsigned char>(view[pos]))) {
            pos++;
        }
        ...
        if (view[pos] == '"') {
            ...
            size_t num_end = pos;
            while (num_end < view.length() && (std::isdigit(static_cast<unsigned char>(view[num_end])) || view[num_end] == '.' || view[num_end] == '-')) {
                num_end++;
            }
            ...
            try {
                ...
                throw std::runtime_error("Invalid numeric value in argument: " + num_str);
            }
            pos = num_end;
        }
        cmd.arguments.push_back(current_arg);
